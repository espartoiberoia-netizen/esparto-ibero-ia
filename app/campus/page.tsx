...
export default function CampusPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (deliveryCourseIds.length > 0) {
        const { data: deliveriesData, error: deliveriesError } =
          await supabase
            ...
            .select(
              "course_id, created_at, ai_reviewed_at, teacher_comment, ai_comment, teacher_file_path"
            )
            .eq("user_id", user.id)
      ...
      // 5️⃣ Mezclar cursos + estado de entrega / revisión
      const coursesWithDeliveries: CampusCourse[] = baseCourses.map((course) => {
        ...
        // Última entrega por fecha de creación
        const sortedByCreated = [...deliveries].sort(
          (a, b) =>
            new Date(b.created_at).getTime() -
            new Date(a.created_at).getTime()
        );
        const lastDeliveryAt = sortedByCreated[0].created_at;

        // Entregas que tienen revisión (IA o profe)
        const reviewedDeliveries = deliveries.filter(
          (d) =>
            d.ai_reviewed_at ||
            d.teacher_comment ||
            d.ai_comment ||
        ...
        if (reviewedDeliveries.length > 0) {
          reviewed = true;
          const sortedReviewed = [...reviewedDeliveries].sort((a, b) => {
            const dateA = new Date(a.ai_reviewed_at ?? a.created_at).getTime();
            const dateB = new Date(b.ai_reviewed_at ?? b.created_at).getTime();
            return dateB - dateA;
          });

          const last = sortedReviewed[0];
          lastReviewedAt = last.ai_reviewed_at ?? last.created_at;
        }
              ...
              >
                <div className="space-y-1">
                  ...
                  <div className="text-xs text-neutral-700">
                    ...
                    ) : course.reviewed && course.lastReviewedAt ? (
                      <span>
                        ✅ Entrega revisada el{" "}
                        {new Date(
